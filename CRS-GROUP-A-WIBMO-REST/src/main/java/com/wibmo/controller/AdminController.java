...
public class AdminController {
	@Autowired
	UserOperationImpl userOperation;
	...
	public ResponseEntity viewPendingApprovals() {
		try {
			return new ResponseEntity(userOperation.viewAccountsPendingForApproval(),HttpStatus.OK);
		}
		catch(Exception e) {
	...
	public ResponseEntity rejectUser(@PathVariable("userId") Integer userId) {
		try {
			if(userOperation.rejectLoginById(userId)) {
	...
	public ResponseEntity approveUser(@PathVariable("userId") Integer userId) {
		try {
			if(userOperation.approveLoginById(userId)) {
